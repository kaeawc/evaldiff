...
func runDiff(args []string, _ io.Writer) error {
	...
	rest := fs.Args()
	if len(rest) != 2 {
		return errors.New("usage: evaldiff diff <base> <head>")
	}
	base, head := rest[0], rest[1]
	return fmt.Errorf("evaldiff diff: not yet implemented (base=%s head=%s)", base, head)
}
...
commands:
  ...
  diff  <base> <head>    Print evals affected between two revisions. (not yet implemented)
